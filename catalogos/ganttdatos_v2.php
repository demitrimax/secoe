...
//echo $querywhere;
 ...
 if ( $result = $database->query( "SELECT * FROM `v_intervenciones` " . $querywhere ) ) {
	if( $result->num_rows > 0 ) {
	  ...
	  echo 'id:'.$contador.'group:'. $row_res['unicoprograma'].'content:'.utf8_encode($row_res['pozo']).'start:'.$fini_ano."-".$fini_mes."-".$fini_dia." 00:00:00", 'end:' .$ffin_ano."-".$ffin_mes."-".$ffin_dia." 23:59:00", 'className:'.$row_res['clasecolor'].'title:'.$tooltip.'<br>';
